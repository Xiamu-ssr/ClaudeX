...
```
停止一个会话（用户点停止 / 切换会话 / 应用退出）会先发 `SIGINT` 尝试真正打断当前正在生成的那一轮，同时关闭 stdin；3 秒内进程仍未退出则升级为 `SIGTERM`。只关闭 stdin 不够——实测确认 CLI 会把当前这一轮跑完才理会 stdin EOF，单纯关 stdin 打不断一次正在进行中的生成，这也是"暂停按钮好像没用"这个用户反馈的真实根因。恢复历史会话用 `--resume <session-id>` 重新 spawn。
...
7. 关闭会话/应用退出 → `SIGINT`（3 秒后升级 `SIGTERM`）+ 关闭 stdin，等真实进程退出才 resolve（`app.on('before-quit')` 触发）；`--resume` 重新 spawn 前必须等旧进程真正退出、`.jsonl` 写完，否则新进程可能读到不完整的历史
...
2. **`process-error`/`process-exited` 事件之前被 `sessionStore.ts` 无条件丢弃**——子进程在还没吐出 `result` 行之前就异常退出或报错（真实崩溃、被杀等），会导致当轮 `isProcessing` 永远卡在 `true`，"..." 指示器永久转下去，没有任何恢复路径。现在这两个事件会解析当前是否有"预期中的停止"（`pendingStopSessionId`，标记用户主动点停止或 `changeModelMidConversation` 内部的停止+重启），预期内的优雅结束成普通态，非预期的则 resolve 成红色错误态——两种情况下 `isProcessing` 都会正确解除。**这里有一个已验证的时序陷阱**：`pendingStopSessionId` 不能在 `stopSession()` 这次 IPC 调用本身 resolve 之后就清空。`ClaudeSession.stop()` 改成等真实进程退出才 resolve 之后，"停止调用完成"和"进程已退出"这两个通知几乎在主进程里同一时刻发出，而实测确认渲染进程会先观察到前者（`invoke` 的响应）、后观察到后者（`webContents.send` 推送的 `process-exited` 事件）——这和"同一个 `exit` 监听器按注册顺序同步触发，推送理应先于 `stop()` 的 promise resolve"这个直觉推理正好相反，是 Electron 里 `invoke`/`send` 两条通道不保证相对顺序的一个实测反例，不能靠理论推导，只能靠实际注入日志验证。修复方式是把清空时机从"`stopSessionInternal` 的 `await` 结束后"整体挪到"`onSessionEvent` 里真正读取并消费这个值的那一刻才清空"，`stopSessionInternal` 自己只在失败路径（`stopSession()` 调用本身抛错，不会有对应的 `process-exited` 来消费）才原地清空兜底。
...
- `main/history/projectOverrides.ts` → `project-overrides.json`，形状 `{ [cwd]: { pinned?: boolean; customName?: string; removed?: boolean; collapsed?: boolean } }`
...
**会话列表排序/刷新时机 + 折叠状态持久化**：`historyReader.ts` 的排序键（`pinned` 优先，组内按 `lastActiveAt` 降序）本身一直是对的，之前被误认为"按创建时间排序"的真正原因是刷新频率——`loadProjectList()` 之前只在挂载时和 6 个 sidecar 类动作（置顶/重命名/移除/归档/删除会话/分叉）之后调用，普通的续聊（`sendMessage`）和新建对话都不会触发它，导致列表顺序长期停留在启动时的快照。现在 `onSessionEvent` 的 `turn-completed` 分支和 `startNewChat` 末尾都会补一次 `loadProjectList()`，让最近活跃的会话/项目及时冒到列表前面。

Sidebar 每个项目默认最多展示 5 条最近会话，超出部分收在一个"显示更多（还有 N 个）"按钮后面（搜索过滤态下不受这个上限约束，直接展示全部匹配项）——这个"展开"状态是纯 UI 态、故意不持久化，每次切换项目/重启都重新收起。这跟项目本身的折叠/展开（点左侧箭头）是两个独立状态：折叠态延续上面 sidecar 的模式，加了 `ProjectOverride.collapsed` 字段和对应的 `setProjectCollapsed` IPC，重启后保留；"显示更多"只是列表长度的本地展开开关，不写 sidecar。
...
推理强度（`--effort low|medium|high|xhigh|max`，真实 CLI 参数）接入**新建对话**和**中途切换模型**这两条路径（`ModelEffortPicker` 在 `mode==='chat'` 时把用户新选的 effort 和 model 一起传给 `changeModelMidConversation`）。store 里 `effortLevel` 的类型是 `EffortLevel | null`，`null` 表示**完全不传 `--effort` 参数**，把决定权交还给 CLI 自身的默认值（可能来自用户真实设置的 `CLAUDE_CODE_EFFORT_LEVEL` 环境变量）——而不是 CCodeBox 用一个编造的默认值静默覆盖它。UI 上对应"CLI 默认"这个选项。

**曾经的真实 bug（已修复）：** `openHistoricalSession`（打开/恢复历史会话，包括 Sidebar 点击会话行）之前完全不传 `model`/`effort`/`extraEnv`，只传了 `resumeSessionId`。实测确认 Claude Code 的 `--resume` **不会**继承该 session 原来 spawn 时用的模型——没有 `--model` 时会回退到全局 `~/.claude/settings.json` 里的 `model` 字段（可能再经用户自己配置的 `ANTHROPIC_DEFAULT_OPUS_MODEL`/`_SONNET_MODEL`/`_HAIKU_MODEL` 等环境变量映射到任意真实后端模型）。加上 `openHistoricalSession` 之前也没有像 `changeModelMidConversation` 一样先 `stop` 掉旧进程，旧进程会被静默孤立（不再被 `SessionManager` 的 Map 追踪，但也没被杀掉，直到应用退出）。触发路径：Sidebar 点击会话行走的是 `handleOpenSession`，对**已经打开的当前会话**再点一次也会无条件重新触发这整套流程——正常使用中很容易无意点到。现在 `openHistoricalSession` 已经和 `startNewChat` 一致，始终带上当前选中的 model/effort/provider env；对同一个已激活的 session 重复调用直接 no-op；切到不同 session 时会先 stop 掉前一个。
...
- 确认过的事实：**Claude Code 本身不内置任何默认 MCP 服务器或 Skill**——它的能力完全来自用户自己配置的 MCP + 可安装的插件市场技能，没有一个"出厂自带、UI 上不会显示"的隐藏集合。所以本节描述的四源数据模型（连接器目录 / 已配置 MCP / 官方技能 / 个人技能）已经是完整覆盖，不存在"漏显示了内置项"的缺口。
...
对话区块的富文本原来是一个手写的简易解析器（只做按 `\n\n` 分段 + 行内单反引号代码），加粗/列表/标题/链接/引用/表格/多行代码块全部不支持——用户实测报告过"`**粗体**` 有时候真的就原样显示两个星号"。换成 `react-markdown` + `remark-gfm`（真正的 CommonMark + GFM 解析），`RichText.tsx` 对外的 `{text, className}` props 签名和具名导出保持不变，调用方（`ChatMessage.tsx` 三处）零改动。主题化通过 `components` prop 逐元素接管渲染，颜色/间距全部复用 `index.css` 里已有的主题 token（`--color-accent-orange`、`--color-card-border` 等），没有新增颜色。
...
9. **应用图标不需要任何新依赖**：`rsvg-convert`/ImageMagick/Inkscape/`cairosvg` 本机全部没装，但 macOS 自带的 `sips` 就能把 SVG（包括渐变）栅格化成任意尺寸 PNG，自带的 `iconutil` 能把一个规范命名的 `.iconset` 目录（`icon_16x16.png`、`icon_16x16@2x.png`……到 `icon_512x512@2x.png`）编译成 `.icns`——全程零新增依赖。**两处要分别配置图标，互不共享**：`packagerConfig.icon` 决定安装后 `.app` 本身的图标（用 `shasum` 对比过打包产物 `Contents/Resources/electron.icns` 和源文件字节一致，确认真的被打包工具采用，不是巧合同名);`MakerDMG` 的 `icon` 选项（`electron-installer-dmg` 的 `ElectronInstallerDMGOptions.icon`，不是 `packagerConfig` 的字段）单独决定 DMG 挂载窗口里显示的图标，漏配这一项不会报错，只会让 DMG 窗口里显示 Electron 默认图标。macOS Big Sur+ 图标约定：内容要先在 SVG 里画成圆角矩形("squircle"，1024 画布内约 824×824 安全区、圆角半径约 185px)——系统不会自动裁剪第三方 App 图标，画错形状就会显示错的形状。
